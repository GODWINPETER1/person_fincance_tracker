    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {Dashboard_data.map((card) => (
          <Card
            key={card.id}
            className={`shadow-lg rounded-xl overflow-hidden transition-transform hover:scale-105 ${
              card.type === "Income"
                ? "bg-green-100"
                : card.type === "Expenses"
                ? "bg-red-100"
          ...
          >
            ...
            <CardFooter className="p-4 text-gray-600 text-sm text-center">{card.footer}</CardFooter>
          </Card>
        ))}
      </div>
